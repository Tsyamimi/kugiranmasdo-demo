...
function selectSpecificBox(boxLabel, event) {
    ...
    // 1. Highlight the specific map box
    const allMapBoxes = document.querySelectorAll('.seat-box');
    allMapBoxes.forEach(b => b.classList.remove('active-box'));
    ...
    // 2. Update the list text
    document.getElementById('vvip-status').innerText = "Selected: " + boxLabel;

    // 3. Highlight the small number in the grid (CRITICAL FIX HERE)
    // Remove class from all spans first
    document.querySelectorAll('#vvip-box-grid span').forEach(s => s.classList.remove('selected-num'));
    
    // Add class to the specific span that was clicked
    event.currentTarget.classList.add('selected-num');

    // 4. Update the checkout bar
    activateCheckout(`VVIP (${boxLabel})`, 1200);
